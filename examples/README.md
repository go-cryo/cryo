...
| `pvc.stagingAccessMode` | no | Access mode of the staging PVC (default `ReadWriteOnce`). `ReadOnlyMany` lets drivers that support it mount the snapshot without cloning it — e.g. ceph-csi CephFS with a StorageClass that sets `backingSnapshot: "true"` (shallow read-only volume). A `ReadWriteOnce` restore from a CephFS snapshot is a full data copy |
